...
func ccw*(a, b, c: Vec2f): bool {.inline.} = orient2d(a, b, c) == orCCW
...
func linePointDistSqr*(p1, p2, p0: Vec2f): float {.inline.} =
...
func distanceLinePointSqr*(p1, p2, p3: Vec2f): (float, float, float, float, float) =
  let
    x1 = p1.x
    y1 = p1.y
    x2 = p2.x
...
#
func lineLineIntersection*(p1, p2, p3, p4: Vec2f): (float, float, float, float) =
  let
    x1 = p1.x
    y1 = p1.y
    x2 = p2.x
...
func unused_dInCircumCircle2(a, b, c, d: Vec2f): bool =
  assert ccw(a, b, c)
  let a1 = a.x - d.x
  let a2 = a.y - d.y
  let b1 = b.x - d.x
...
func dInCircumCircle*(a, b, c, d: Vec2f): bool =
...
func interpolateZ*(p1, p2, p3: Vec3f; p: var Vec3f) =
...
func interpolateZ*(p1, p2: Vec3f; p: var Vec3f) =
...
# https://en.wikipedia.org/wiki/Vec2f_projection
#
#      /P2
#     /
#    /\
...
# z = u * (z2 - z1)
#
func projection*(p1, p2, p0: Vec2f): Vec2f {.inline.} =
...
#}
#
func pnpoly*(p: openArray[Vec2f]; x, y: float): bool {.inline.} =
  var i, j: int
  j = p.high
  while i < p.len:
