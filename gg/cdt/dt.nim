...
import gg/vectors
import geometry
import types, vertices, edges, subdivisions, salewski, minmax
...
#   /
# a ----- p
func getLeftEdge(a: Vertex; p: Vec2f): Edge =
  result = a.firstEdge
...
# https://www.researchgate.net/publication/2478154_Fully_Dynamic_Constrained_Delaunay_Triangulations
proc flipEdges(p: Vec2f; stack: var seq[Edge]) =
  while stack.len > 0:
    let e = stack.pop
...
proc initDelaunayTriangulation*(a, b: Vec2f; precision = 1e-8): DelaunayTriangulation =
...
func rightOf(x: Vec2f; e: Edge): bool {.inline.} =
...
proc xlocatePoint(p: Vec2f; startEdge: Edge): Edge =
  var e = startEdge
  assert e != nil
  ...
  let a = orient2d(e.org.point, e.dest.point, p)
  assert a <= orCollinear
  if a == orCCW:
...
proc locatePoint(this: DelaunayTriangulation; p: Vec2f; startEdge: Edge = nil): (Edge, LocPos) =
  var e = startEdge
  if e == nil:
    e = this.getStartVertex(p.x, p.y).edge
  e = xlocatePoint(p, e)
  var pos: LocPos
  assert orient2d(e.org.point, e.dest.point, p) <= orCollinear
...
proc insertPointInEdge(this: var DelaunayTriangulation; point: Vec2f; edge: Edge): Vertex =
  var p = point
  var e = edge
...
proc insertPointInFace(this: var DelaunayTriangulation; p: Vec2f; edge: Edge): Vertex =
...
proc insertPoint*(this: var DelaunayTriangulation; p: Vec2f): Vertex =
...
proc splitEdge(this: var DelaunayTriangulation; point: Vec2f; edge: Edge): Vertex =
...
proc rangeCheck(this: DelaunayTriangulation; v: Vec2f) =
  assert(v.x > this.xmin and v.x < this.xmax and v.y > this.ymin and v.y < this.ymax)

proc insert*(this: var DelaunayTriangulation; points: varargs[Vec2f]): ConstraintID =
  var lastVert: Vertex
